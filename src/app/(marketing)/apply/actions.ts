  ...
  try {
    ...
    if (insertErr || !row) {
      ...
      if (insertErr?.message?.includes("does not exist")) {
        return {
          error:
            "Applications database is not set up yet. Run supabase/RUN_IN_SUPABASE.sql in the Supabase SQL editor.",
        };
      }
    ...
    if (/Missing NEXT_PUBLIC_SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY/i.test(msg)) {
      return {
        error:
          "Application form is not configured on the server yet. Add SUPABASE_SERVICE_ROLE_KEY in Vercel, then redeploy.",
      };
    }
